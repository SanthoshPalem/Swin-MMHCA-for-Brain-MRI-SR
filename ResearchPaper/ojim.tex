...
\author{Santhosh Palem\orcidlink{0009-0004-0044-4143}, Jagadeesh Ospari\,\orcidlink{0009-0005-4536-6844}, Chakravarthi Padyala\,\orcidlink{0009-0000-2223-5140}, Syed Sameen Ahmad Rizvi \orcidlink{0000-0002-3919-5074} (MEMBER, IEEE), Priya K V \orcidlink{0000-0001-5552-0351} (SENIOR MEMBER, IEEE)}
...
\begin{enumerate}
    ...
    \item Extensive validation on the IXI dataset, demonstrating superior performance in terms of PSNR and SSIM compared to existing CNN-based and multi-modal baselines.
\end{enumerate}
...
We assess performance using Peak Signal-to-Noise Ratio (PSNR) and Structural Similarity Index (SSIM). Furthermore, we evaluate the perceptual quality using the Learned Perceptual Image Patch Similarity (LPIPS) metric (with an AlexNet backbone), where a lower value indicates better performance. Our Swin-MMHCA model achieves a superior LPIPS score of 0.0607, demonstrating high perceptual fidelity. Table \ref{tab:results} summarizes the comparative results on the IXI test set for $2\times$ and $4\times$ upscaling.

\begin{table}[H]
\centering
\caption{PSNR and SSIM scores of various state-of-the-art methods on the IXI dataset for the T2w target modality. For two of the existing methods (EDSR and CNN+ESPC), we evaluate versions enhanced with various attention modules. Our results (Swin-MMHCA) demonstrate the superior performance of the hierarchical transformer architecture.}
\label{tab:results}
\resizebox{\columnwidth}{!}{
\begin{tabular}{lcccc}
\toprule
...
EDSR + CSAM \cite{edsr, minet, han} & 39.81 & 0.9865 & 31.83 & 0.9377 \\
EDSR + CBAM \cite{edsr, cbam} & 39.82 & 0.9865 & 31.81 & 0.9374 \\
EDSR + MHCA \cite{edsr, Georgescu2023} & 40.11 & 0.9871 & 32.15 & 0.9418 \\
EDSR + MCSAM \cite{edsr, minet, han} & 40.12 & 0.9871 & 32.17 & 0.9417 \\
EDSR + MCBAM \cite{edsr, cbam} & 40.13 & 0.9871 & 32.18 & 0.9421 \\
EDSR + MMHCA \cite{edsr, Georgescu2023} & 40.28 & 0.9874 & 32.51 & 0.9452 \\
EDSR + MMHCA+ \cite{edsr, Georgescu2023} & \textcolor{red}{40.43} & \textcolor{red}{0.9877} & \textcolor{red}{32.70} & \textcolor{blue}{0.9469} \\
\midrule
CNN+ESPC \cite{georgescu2020} & 38.67 & 0.9837 & 30.57 & 0.9210 \\
CNN+ESPC + CSAM \cite{georgescu2020, minet, han} & 38.57 & 0.9835 & 30.58 & 0.9211 \\
CNN+ESPC + CBAM \cite{georgescu2020, cbam} & 38.67 & 0.9838 & 30.47 & 0.9192 \\
CNN+ESPC + MHCA \cite{georgescu2020, Georgescu2023} & 39.04 & 0.9847 & 30.76 & 0.9233 \\
CNN+ESPC + MCSAM \cite{georgescu2020, minet, han} & 38.98 & 0.9845 & 30.94 & 0.9265 \\
CNN+ESPC + MCBAM \cite{georgescu2020, cbam} & 38.91 & 0.9844 & 30.79 & 0.9238 \\
CNN+ESPC + MMHCA \cite{georgescu2020, Georgescu2023} & 39.71 & 0.9862 & 31.52 & 0.9337 \\
CNN+ESPC + MMHCA+ \cite{georgescu2020, Georgescu2023} & 39.76 & 0.9863 & 31.52 & 0.9337 \\
\midrule
\rowcolor{gray!10} \textbf{Swin-MMHCA (Ours)} & \textcolor{red}{\textbf{40.62}} & \textcolor{red}{\textbf{0.9927}} & \textcolor{red}{\textbf{33.32}} & \textcolor{red}{\textbf{0.9572}} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{table}[H]
\centering
\caption{Ablation study of Swin-MMHCA components on IXI dataset.}
\label{tab:ablation}
\resizebox{\columnwidth}{!}{
\begin{tabular}{l|ccc|cc}
\toprule
\textbf{Configuration} & \textbf{MMHCA} & \textbf{Edge} & \textbf{Multi-Task} & \textbf{PSNR (2$\times$)} & \textbf{PSNR (4$\times$)} \\
\midrule
Unimodal SwinV2 & & & & 38.12 & 31.14 \\
+ Multi-modal Concatenation & & & & 38.65 & 31.52 \\
+ MMHCA Module & \checkmark & & & 40.48 & 32.90 \\
+ Edge Guidance & \checkmark & \checkmark & & 40.55 & 33.11 \\
\textbf{Full Swin-MMHCA} & \checkmark & \checkmark & \checkmark & \textbf{40.62} & \textbf{33.32} \\
\bottomrule
\end{tabular}
}
\end{table}

The results in Table \ref{tab:ablation} show that while multi-modal concatenation provides a baseline improvement, the MMHCA module provides the most substantial boost (+1.83 dB at 2$\times$) by dynamically aligning features across modalities. The Edge Guidance branch further refines the structural boundaries, while the Multi-Task heads provide the final gain (+0.21 dB at 4$\times$), likely by regularizing the backbone to learn anatomically consistent features.

\subsection{Qualitative Analysis}
Visual inspection of the SR outputs reveals that Swin-MMHCA effectively recovers the intricate folding patterns of the cerebral cortex and the sharp boundaries of the ventricles, as demonstrated in Fig. \ref{fig:results}.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{Paper_Comparison_Visuals/comparison_figure_x4.png}
\caption{Visual comparison of $4\times$ SR results. From left to right: LR Input, Bicubic, EDSR-MMHCA (Baseline), Swin-MMHCA (Ours), and HR Ground Truth. The zoom-in boxes highlight the recovery of cortical textures and structural details.}
\label{fig:results}
\end{figure*}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{SP.png}}]
{Santhosh Palem} is currently pursuing his fourth year of a Bachelor of Technology (B.Tech) in Computer Science and Engineering at SRM University - AP, with an expected graduation in 2026. His research interests include medical image super-resolution, deep learning, and computer vision. He is passionate about developing intelligent systems that enhance diagnostic accuracy in healthcare.
\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{JO.png}}]{Jagadeesh Ospari} is currently pursuing his fourth year of a Bachelor of Technology (B.Tech) in Computer Science and Engineering at SRM University - AP, with an expected graduation in 2026. His research focuses on deep learning architectures and hierarchical vision transformers for medical image analysis. He is particularly interested in leveraging multi-modal data for robust image enhancement.
\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{CP.png}}]{Chakravarthi Padyala} is currently pursuing his fourth year of a Bachelor of Technology (B.Tech) in Computer Science and Engineering at SRM University - AP, with an expected graduation in 2026. His academic interests include computer vision, attention mechanisms, and multi-modal feature fusion. He enjoys exploring how data-driven models can be optimized for clinical utility and anatomical fidelity.
\end{IEEEbiography}
